...
final class Updatronix_AutoUpdateDelay {
    ...
    public static function register_filters(): void {
        ...
        add_filter('auto_update_plugin', [self::class, 'filter_plugin'], self::FILTER_PRIORITY, 2);
        add_filter('auto_update_theme', [self::class, 'filter_theme'], self::FILTER_PRIORITY, 2);
        add_filter('auto_update_core', [self::class, 'filter_core'], self::FILTER_PRIORITY, 2);
        add_filter('auto_update_translation', [self::class, 'filter_translation'], self::FILTER_PRIORITY, 2);
    }
    ...
    private static function delay_gate_should_run(): bool {
        if (self::$delay_enabled_cache !== null) {
        ...
        self::$delay_settings_slice = [
            'enabled' => $enabled,
            'days' => max(1, min(365, $days ?: 1)),
        ];
    ...
    private static function maybe_prune_ledger(): void {
        ...
        if ($core_bundle instanceof \stdClass && !empty($core_bundle->updates) && is_array($core_bundle->updates)) {
            foreach ($core_bundle->updates as $core_offer) {
                ...
                if (isset($core_offer->response) && $core_offer->response === 'autoupdate') {
    ...
    private static function log_deferred_offer(string $type, object $item, int $first_seen_unix, int $delay_seconds): void {
        $slice = self::$delay_settings_slice ?? ['enabled' => false, 'days' => 7];
        ...
        $log_type_safe = ($type === 'translation') ? 'translation' : $type;
    ...
    private static function resolve_offer_dimensions(string $type, object $item): array {
        switch ($type) {
            case 'plugin':
                ...
                if ('' === $file) {
                    return [__('Unknown plugin', 'updatronix'), '', '', (string) ($item->new_version ?? '')];
                }
            ...
            case 'theme':
                ...
                if ('' === $stylesheet) {
                    return [__('Unknown theme', 'updatronix'), '', '', (string) ($item->new_version ?? '')];
                }
