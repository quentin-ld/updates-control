...
final class Updatronix_Export_Body_Builder {
	...
	/**
	 * Coerce a raw `columns` toggle value to a boolean.
	 *
	...
	/**
	 * Filter row keys down to those enabled by the current column toggles.
	 *
	...
	/**
	 * Whether the table heading row should be rendered.
	 *
	 * @param array<string, bool> $columns Normalised column toggles.
	 * @return bool
	 */
	...
	/**
	 * Whether category section headings should be rendered.
	 *
	 * @param array<string, bool> $columns Normalised column toggles.
	 * @return bool
	 */
	...
	/**
	 * Append records to the chunk body until the byte cap is reached.
	 *
	...
	private static function build_merged_sectioned_records( array $buckets, array $columns ): array {
		/**
		 * Sort-ready row data grouped by category.
		 *
		...
		/**
		 * Flattened list of all sectioned items, used to compute shared column widths.
		 *
	...
	private static function user_label_for_row( object $row ): string {
		...
		if ( 'system' === $performed_by || 0 >= $user_id ) {
	...
	private static function trigger_context_suffix( array $group ): string {
		...
		$context = '';
		if ( $all_have_context && 1 === count( $contexts ) ) {
	...
	/**
	 * Format a Unix timestamp using the site's date/time preferences.
	 *
