...
final class Updatronix_Security {
	 ...
	 * @var array<string>
	 */
	public const ALLOWED_ACTION_TYPES = array( 'update', 'downgrade', 'install', 'same_version', 'failed', 'uninstall' );
	 ...
	 * @var array<string>
	 */
	public const ALLOWED_STATUSES = array( 'success', 'error', 'cancelled' );
	...
	public static function sanitize_version( string $value ): string {
		...
		return mb_substr( $sanitized ? $sanitized : '', 0, 64 );
	}
	...
	private static function redact_sensitive_text( string $value ): string {
		$redacted = preg_replace( '/([?&](?:token|signature|sig|key|access_token|auth)[^=\s]*=)[^&\s]+/i', '$1[redacted]', $value );
