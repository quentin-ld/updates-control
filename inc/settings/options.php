...
const UPDATRONIX_SETTINGS_DEFAULTS = array(
	...
	'retention_days' => 90,
	/**
	 * Sanitized via updatronix_sanitize_notifications_mode().
	 *
...
function updatronix_get_allowed_cron_schedule_labels(): array {
	/**
	 * Registered cron schedules, keyed by slug.
	 *
 ...
 *
 * @since 1.1.0
 * @param array<string, mixed> $partial  Partial Schedule payload from REST.
 * @param array<string, mixed> $baseline Already-normalized baseline Schedule values.
 * @return array<string, mixed>
 */
 ...
 *
 * @since 1.1.0
 * @param array<string, mixed> $in Raw Schedule subtree.
...
function updatronix_maybe_grant_manage_cap(): void {
	...
	if ( '1' === get_option( 'updatronix_cap_migrated', '' ) ) {
		return;
	}
...
function updatronix_sanitize_emails( mixed $value ): string {
	$raw = (string) $value;
	if ( UPDATRONIX_NOTIFY_EMAILS_MAX_BYTES < strlen( $raw ) ) {
	...
	foreach ( $candidates as $candidate ) {
		...
		$emails[] = $clean;
		if ( UPDATRONIX_NOTIFY_EMAILS_MAX_RECIPIENTS <= count( $emails ) ) {
			break;
		}
	}
