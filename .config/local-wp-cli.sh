...
case "$MODE" in
	...
	setup)
		...
		if [[ ! -f "$ENV_FILE" || "$FORCE" == "--force" ]]; then
			DB_NAME="$(wp config get DB_NAME 2>/dev/null || echo local)"
			DB_USER="$(wp config get DB_USER 2>/dev/null || echo root)"
			DB_PASS="$(wp config get DB_PASSWORD 2>/dev/null || echo root)"
			DB_HOST="$(wp config get DB_HOST 2>/dev/null || echo localhost)"
