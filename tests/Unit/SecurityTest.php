...
final class SecurityTest extends TestCase {
    ...
    public static function provide_token_redaction(): array {
        return [
            'token' => ['?token=secret', '?token=[redacted]'],
            'signature' => ['?signature=abc123', '?signature=[redacted]'],
            'sig' => ['?sig=xyz', '?sig=[redacted]'],
            'key' => ['?key=val', '?key=[redacted]'],
            'access_token' => ['?access_token=eyJ', '?access_token=[redacted]'],
            'auth' => ['?auth=key', '?auth=[redacted]'],
            'auth_token' => ['?auth_token=xyz', '?auth_token=[redacted]'],
        ];
    }
    ...
    public static function provide_auth_false_positive(): array {
        return [
            'author param' => ['?author=John'],
            'authorization param' => ['?authorization=Bearer x'],
            'authorize param' => ['?authorize=true'],
            'authentication param' => ['?authentication=basic'],
        ];
    }
    ...
    public static function provide_path_redaction(): array {
        return [
            'abspath-root' => ['/tmp/index.php (42)', 'tmp[redacted] (42)'],
            'abspath-nested' => ['/tmp/wp-admin/admin.php', 'tmp[redacted]'],
            'plugin-dir' => ['/tmp/plugins/akismet/akismet.php', 'tmp[redacted]'],
            'content-dir' => ['/tmp/wp-content/themes/twentytwentyfour/style.css', 'tmp[redacted]'],
        ];
    }
