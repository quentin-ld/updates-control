...
final class MultisiteNetworkOnlyTest extends WP_UnitTestCase {
    ...
    public function test_deleting_a_site_purges_its_log_rows(): void {
        ...
        $wpdb->update($table, ['site_id' => $foreign_site_id], ['item_slug' => 'doomed'], ['%d'], ['%s']);

        self::assertSame(1, Updatronix_Logger::get_logs_count(['site_id' => $foreign_site_id]));

        Updatronix_Logger::delete_logs_for_site($foreign_site_id);

        self::assertSame(0, Updatronix_Logger::get_logs_count(['site_id' => $foreign_site_id]));
    }
    ...
    public function test_uninstall_clears_network_schedule_site_option(): void {
        ...
        require_once updatronix_PLUGIN_DIR . 'inc/classes/Uninstall.php';
